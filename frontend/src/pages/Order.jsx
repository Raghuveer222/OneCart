  ...
  const loadOrderData = async () => {
    try {
      ...
      if (result.data && result.data.success) {
        ...
        // Ensure we are accessing the 'orders' array specifically
        const ordersArray = result.data.orders || [];

        ordersArray.forEach((order) => {
          // Ensure 'items' exists inside each order
          if (order.items && Array.isArray(order.items)) {
            order.items.forEach((item) => {
              // Merging order details into the item object for the UI
              ...
              allOrdersItem.push(itemWithDetails);
            });
          }
        });

        // Update state with the flattened list
